"""
Eval metrics for each sample: root-aligned MPJPE, mean relative root position
error (right vs left), and pixel error.
"""
import numpy as np
import torch
...
def mpjpe(pred_j3d: torch.Tensor, gt_j3d: torch.Tensor, per_joint_valid: torch.Tensor) -> np.ndarray:
    """
...
def mrrpe(pred_root_r: torch.Tensor, pred_root_l: torch.Tensor, gt_root_r: torch.Tensor, gt_root_l: torch.Tensor, valid: torch.Tensor) -> np.ndarray:
    """
