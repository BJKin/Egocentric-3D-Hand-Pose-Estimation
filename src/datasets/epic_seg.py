"""
EPIC-Kitchens VISOR segmentation dataset.
"""
import pickle
import random
...
class EPICSegDataset(Dataset):
    def __init__(self, split: str):
        """
        Loads the VISOR annotations, bboxes, and predicted hand masks.

        Arguments:
            split -- 'train'
        """
        assert split == "train", f"EPICSegDataset only supports the 'train' split, got {split!r}"
        with open(EPIC_HANDS_DIR / "modal_amodal_annot.pkl", "rb") as f:
            self.modal = pickle.load(f)
        with open(EPIC_HANDS_DIR / "grasp_visor_train.pkl", "rb") as f:
            self.bbox = pickle.load(f)

        self.masks_path = EPIC_HANDS_DIR / "visor_pred_masks_train.npz"
        with np.load(self.masks_path, allow_pickle=True) as npz:
            mask_keys = set(npz.files)
        self.masks_npz = None
        self.imgnames = sorted(set(self.modal) & set(self.bbox) & mask_keys)

        self.aug_data = True
        self.normalize_img = Normalize(mean=IMG_NORM_MEAN, std=IMG_NORM_STD)
        self.intrx = dummy_egocam_intrx(VISOR_W, VISOR_H)
        logger.info(f"# samples in EPIC seg train: {len(self.imgnames)}")
    ...
    def __getitem__(self, idx: int) -> Tuple[dict, dict, dict]:
        ...
        modal_labels = {"left": 0, "right": 0, **self.modal[pickle_key]}
        ...
        mask_augm = {**augm, "pn": np.ones_like(augm["pn"])}  # no color noise on masks
        ...
        targets = {
            ...
            "grasp.r": 8, "grasp.l": 8,                       # no grasp labels
            "grasp_valid_r": 0, "grasp_valid_l": 0,
            "render.r": torch.from_numpy(mask_img_r[0:1]).float(),  # all 3 channels are identical
            "render.l": torch.from_numpy(mask_img_l[0:1]).float(),
            "render_valid_r": float(modal_labels["right"] == 1) * right_valid,
            "render_valid_l": float(modal_labels["left"] == 1) * left_valid,
            "is_valid": 1.0,
    ...
    def _bbox_clipped_mask(mask_npz: np.ndarray, pixel_val: int, bbox_og: np.ndarray, bbox_present: bool) -> np.ndarray:
        ...
        Arguments:
            ...
            bbox_present -- if False, returns an all-zero mask (no hand to render)

        Returns:
            mask -- HxWx3 uint8-compatible float mask in {0, 255}
        """
        if not bbox_present:
