"""
Wave-ViT backbone (vendored).

Source: Yao et al., "Wave-ViT: Unifying Wavelet and Transformers for Visual
Representation Learning", ECCV 2022 -- https://github.com/YehLi/ImageNetModel
(classification/wavevit.py), Apache-2.0 license.

Vendored as a swappable backbone for this WildHands reimplementation. The only
local changes are import-line fixes (timm import paths made resilient to modern
timm; the sibling wavelet import points at src.torch_wavelets). Model logic is
...
except ImportError:  # _cfg not exported in this timm; we don't use timm pretrained
    def _cfg(**kwargs):
        return {}
import math
import numpy as np
from src.torch_wavelets import DWT_2D, IDWT_2D
