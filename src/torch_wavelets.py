"""
2D discrete wavelet transform ops for Wave-ViT (vendored).

Source: https://github.com/YehLi/ImageNetModel (classification/torch_wavelets.py),
Apache-2.0 license. One local change: the DWT/IDWT filter buffers are kept in
float32 (upstream downcast them to float16 for half/AMP training); this pipeline
runs float32 with no autocast, so the cast is removed to avoid a dtype mismatch.
"""
