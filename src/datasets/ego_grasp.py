...
from src.datasets.epic_grasp import NO_GRASP, _GRASP_LABEL
...
class Ego4DGraspDataset(Dataset):
    def __init__(self, split: str):
        """
        Loads the Ego4D grasp annotations.

        Arguments:
            split -- 'train'
        """
        assert split == "train", f"Ego4DGraspDataset only supports the 'train' split, got {split!r}"
        ...
        logger.info(f"# samples in Ego4D grasp train: {len(self.imgnames)}")
